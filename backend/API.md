...
**参数说明**:
- `rarity`: 稀有度 (C, B, A, S)
- `price`: 单价（种子买入价：C=10, B=30, A=50, S=100）
...
**参数说明**:
- `rarity`: 稀有度 (C, B, A, S)
- `price`: 单价（根据稀有度：C=20, B=60, A=100, S=200，肥料买卖价格相同）
- `quantity`: 购买数量 (默认1，最大99)
...
- `SELL_SEED`: 卖出种子（基于同品质最低购买价的50%）
- `SELL_USE`: 卖出肥料（基于同品质最低购买价的50%）
- `SELL_CROP`: 卖出作物（基于同品质种子最高价的200%）
...
**查询参数**:
- `itemType`: 物品类型 (seed, use, crop)
- `rarity`: 稀有度 (C, B, A, S)

**价格计算规则**:
- **种子/肥料**: 卖出价 = 同品质物品最低购买价 × 50%（向下取整）
- **作物**: 卖出价 = 同品质种子最高购买价 × 200%
