...
async function calculateSellPrice(itemType, rarity) {
  ...
  if (itemType === 'crop') {
    ...
    const sellPrice = Math.min(maxPrice * 2, MAX_PRICE);
...
const MAX_POINTS = 999999999; // 积分上限(约10亿)
const MAX_PRICE = 9999; // 商品价格上限

app.post('/api/user/seeds', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const { rarity, quantity = 1 } = req.body;
    ...
    const priceResult = await client.query(
      'SELECT MIN(price) as min_price FROM plants WHERE rarity = $1 AND plants_role = $2',
      [rarity, 'seed']
    );
    const price = priceResult.rows[0]?.min_price;
    if (!price) {
      return res.status(400).json({ error: '该稀有度种子不存在' });
    }

    const buyQuantity = Math.max(1, Math.min(parseInt(quantity) || 1, 999));
    ...
    try {
      ...
      // 扣除积分
      const updatedPoints = Number(user.points) - totalPrice;
...
app.post('/api/user/uses', authenticateToken, async (req, res) => {
  try {
    ...
    if (!rarity) {
      console.log('Missing rarity');
      return res.status(400).json({ error: '请提供物品稀有度' });
    }
    ...
    const price = priceResult.rows[0]?.min_price;
    if (!price) {
      return res.status(400).json({ error: '该稀有度物品不存在' });
    }
...
app.delete('/api/user/uses/:rarity', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      let updatedPoints = Number(user.points || 0) + totalPrice;
