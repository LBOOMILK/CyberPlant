...
router.post('/decorations', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const result = await client.query('INSERT INTO decorations (name, icon, slot_type, quality, bonus, price_type, price_amount, pet_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *', [name, icon||'🎀', slot_type, quality||'C', bonus||0, price_type||'silver_coin', price_amount||100, pet_id||null]);
    res.status(201).json(result.rows[0]);
  } catch (error) { res.status(500).json({ error: '创建饰品失败' }); }
});

router.put('/decorations/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params; const { name, icon, slot_type, quality, bonus, price_type, price_amount, pet_id } = req.body;
    ...
    const e = existing.rows[0];
    const result = await client.query('UPDATE decorations SET name=$1, icon=$2, slot_type=$3, quality=$4, bonus=$5, price_type=$6, price_amount=$7, pet_id=$8 WHERE id=$9 RETURNING *', [name||e.name, icon||e.icon, slot_type||e.slot_type, quality||e.quality, bonus||e.bonus, price_type||e.price_type, price_amount||e.price_amount, pet_id!==undefined?pet_id:e.pet_id, id]);
    res.json(result.rows[0]);
  } catch (error) { res.status(500).json({ error: '更新饰品失败' }); }
});

router.delete('/decorations/:id', authenticateToken, requireAdmin, async (req, res) => { try { const result = await client.query('DELETE FROM decorations WHERE id = $1 RETURNING *', [req.params.id]); if (result.rowCount === 0) return res.status(404).json({ error: '饰品不存在' }); res.json({ message: '饰品删除成功' }); } catch (error) { res.status(500).json({ error: '删除饰品失败' }); } });
