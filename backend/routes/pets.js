...
router.post('/user/pets/:userPetId/feed', authenticateToken, async (req, res) => {
  try {
    ...
    if (currentHunger + foodEffect.hunger > hungerMax && !confirm_overflow) return res.status(400).json({ overflow: true, waste: currentHunger + foodEffect.hunger - hungerMax, message: `饱食度将溢出 ${currentHunger + foodEffect.hunger - hungerMax} 点，确认继续？` });
    await client.query('BEGIN');
    try {
      ...
      res.json({ message: '喂食成功', pet: await formatPetData(updatedPet.rows[0], updatedPet.rows[0]), fed_food: foodItem.rows[0].name, growth_gained: foodEffect.growth, leveled_up: newLevel > pet.level ? newLevel : false });
    } catch (err) { await client.query('ROLLBACK'); throw err; }
