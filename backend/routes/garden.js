...
const { client, logger, getConfig, getUserItemCount, getPetBonus } = require('../db');
...
router.get('/user/plots', authenticateToken, async (req, res) => {
  try {
    ...
    if (plotsResult.rowCount === 0) {
      ...
      try { for (let i = 1; i <= 6; i++) await client.query('INSERT INTO garden_plots (user_id, plot_index, is_unlocked) VALUES ($1,$2,$3)', [userId, i, i === 1]); await client.query('COMMIT'); } catch (err) { await client.query('ROLLBACK'); throw err; }
    ...
    for (const plot of plotsResult.rows) {
      let cropInfo = null;
      if (plot.seed_id) { const r = await client.query('SELECT id, name, icon, rarity, item_type, base_yield, water_cd FROM items WHERE id = $1', [plot.seed_id]); if (r.rowCount > 0) cropInfo = r.rows[0]; }
      plots.push({ id: plot.id, plot_index: plot.plot_index, is_unlocked: plot.is_unlocked, level: plot.level, seed_id: plot.seed_id, stage: plot.stage, planted_at: plot.planted_at, last_watered_at: plot.last_watered_at, is_dead: plot.is_dead || false, fertilized: plot.fertilized || false, crop: cropInfo ? { id: cropInfo.id, name: cropInfo.name, icon: cropInfo.icon, rarity: cropInfo.rarity, base_yield: cropInfo.base_yield, water_cd: Math.min(240, cropInfo.water_cd || 5) } : null, stage_icon: plot.seed_id ? (plot.stage < 4 ? STAGE_ICONS[plot.stage] : (cropInfo ? cropInfo.icon : '🌿')) : null, multiplier: PLOT_LEVEL_MULTIPLIER[plot.level] || 1.0 });
    }
    res.json({ plots });
  } catch (error) { logger.error('Get plots error', { error: error.message }); res.status(500).json({ error: '获取地块信息失败' }); }
});

router.post('/user/plots/:plotIndex/unlock', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id; const plotIndex = parseInt(req.params.plotIndex);
    if (isNaN(plotIndex) || plotIndex < 2 || plotIndex > 6) return res.status(400).json({ error: '地块序号无效' });
    const cost = await getUnlockCost(plotIndex); if (!cost) return res.status(400).json({ error: '该地块无法解锁' });
    ...
    const { deductCurrency: deduct, createOrder } = require('../db');
    await client.query('BEGIN');
    try { await deduct(userId, cost.type, cost.amount); await client.query('UPDATE garden_plots SET is_unlocked = true WHERE user_id = $1 AND plot_index = $2', [userId, plotIndex]); await createOrder(userId, 'PLOT_UNLOCK', cost.type, cost.amount); await client.query('COMMIT'); } catch (err) { await client.query('ROLLBACK'); if (err.message === 'Insufficient balance') return res.status(400).json({ error: '余额不足' }); throw err; }
  ...
  } catch (error) { logger.error('Unlock plot error', { error: error.message }); res.status(500).json({ error: '解锁地块失败' }); }
});

router.post('/user/plots/:plotIndex/upgrade', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id; const plotIndex = parseInt(req.params.plotIndex);
    ...
    const cost = await getUpgradeCost(nextLevel); if (!cost) return res.status(400).json({ error: '无法继续升级' });
    const { deductCurrency: deduct, createOrder } = require('../db');
    await client.query('BEGIN');
    try { await deduct(userId, cost.type, cost.amount); await client.query('UPDATE garden_plots SET level = $1 WHERE user_id = $2 AND plot_index = $3', [nextLevel, userId, plotIndex]); await createOrder(userId, 'PLOT_UPGRADE', cost.type, cost.amount); await client.query('COMMIT'); } catch (err) { await client.query('ROLLBACK'); if (err.message === 'Insufficient balance') return res.status(400).json({ error: '余额不足' }); throw err; }
    const cur = await client.query('SELECT silver_coin, gold_coin, diamond FROM currencies WHERE user_id = $1', [userId]);
  ...
  } catch (error) { logger.error('Upgrade plot error', { error: error.message }); res.status(500).json({ error: '升级地块失败' }); }
});

router.post('/user/plots/:plotIndex/plant', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id; const plotIndex = parseInt(req.params.plotIndex); const { item_id } = req.body;
    if (isNaN(plotIndex) || plotIndex < 1 || plotIndex > 6) return res.status(400).json({ error: '地块序号无效' });
    if (!item_id) return res.status(400).json({ error: '请提供种子 item_id' });
    const plotResult = await client.query('SELECT * FROM garden_plots WHERE user_id = $1 AND plot_index = $2', [userId, plotIndex]);
    if (plotResult.rowCount === 0) return res.status(404).json({ error: '地块不存在' });
    if (!plotResult.rows[0].is_unlocked) return res.status(400).json({ error: '地块未解锁' });
    if (plotResult.rows[0].seed_id !== null) return res.status(400).json({ error: '地块已有植物' });
    ...
    res.json({ message: '种植成功', plot_index: plotIndex, crop: { id: cropInfo.id, name: cropInfo.name, icon: cropInfo.icon, rarity: cropInfo.rarity, base_yield: cropInfo.base_yield } });
  } catch (error) { logger.error('Plant error', { error: error.message }); res.status(500).json({ error: '种植失败' }); }
});

router.post('/user/plots/:plotIndex/water', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id; const plotIndex = parseInt(req.params.plotIndex);
    ...
    if (plot.is_dead) return res.status(400).json({ error: '植物已死亡' });
    const waterCd = Math.min(240, plot.water_cd || 5);
    const elapsed = (Date.now() - new Date(plot.planted_at).getTime()) / 1000;
    if (elapsed > 180) { await client.query('UPDATE garden_plots SET is_dead = true WHERE user_id = $1 AND plot_index = $2', [userId, plotIndex]); return res.status(400).json({ error: '植物已因缺水而死亡' }); }
    if (plot.stage >= 4) { await client.query('UPDATE garden_plots SET planted_at = CURRENT_TIMESTAMP WHERE user_id = $1 AND plot_index = $2', [userId, plotIndex]); return res.json({ message: '浇水成功', plot_index: plotIndex, stage: plot.stage, is_mature: true }); }
    if (elapsed < waterCd) return res.status(400).json({ error: `浇水冷却中，还需 ${Math.ceil(waterCd - elapsed)} 秒` });
    ...
    res.json({ message: newStage >= 4 ? '🎉 植物成熟了！' : '💧 浇水成功', plot_index: plotIndex, stage: newStage, is_mature: newStage >= 4 });
  } catch (error) { logger.error('Water error', { error: error.message }); res.status(500).json({ error: '浇水失败' }); }
});

router.post('/user/plots/:plotIndex/fertilize', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id; const plotIndex = parseInt(req.params.plotIndex); const { item_id } = req.body;
    if (!item_id) return res.status(400).json({ error: '请提供肥料 item_id' });
    const plotResult = await client.query('SELECT gp.*, i.name as crop_name, i.icon as crop_icon FROM garden_plots gp LEFT JOIN items i ON gp.seed_id = i.id WHERE gp.user_id = $1 AND gp.plot_index = $2', [userId, plotIndex]);
    if (plotResult.rowCount === 0) return res.status(404).json({ error: '地块不存在' });
    const plot = plotResult.rows[0];
    if (!plot.seed_id || !plot.is_unlocked) return res.status(400).json({ error: '条件不满足' });
    if (plot.is_dead) return res.status(400).json({ error: '植物已死亡' });
    if (plot.stage >= 4) return res.status(400).json({ error: '植物已成熟' });
    if (plot.fertilized) return res.status(400).json({ error: '该作物已使用过肥料' });
    const itemResult = await client.query('SELECT * FROM items WHERE id = $1', [item_id]);
    if (itemResult.rowCount === 0 || itemResult.rows[0].item_type !== 'fertilizer') return res.status(400).json({ error: '无效的肥料' });
    const stageSkip = itemResult.rows[0].stage_skip || 1;
    const userItemResult = await client.query('SELECT * FROM user_items WHERE user_id = $1 AND item_id = $2', [userId, item_id]);
    ...
    try {
      ...
      await client.query('COMMIT');
      res.json({ message: newStage >= 4 ? '🎉 施肥后植物直接成熟了！' : `🧪 施肥成功！跳过 ${stageSkip} 个阶段`, plot_index: plotIndex, stage: newStage, is_mature: newStage >= 4 });
    } catch (err) { await client.query('ROLLBACK'); throw err; }
  } catch (error) { logger.error('Fertilize error', { error: error.message }); res.status(500).json({ error: '施肥失败' }); }
});

router.post('/user/plots/:plotIndex/harvest', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id; const plotIndex = parseInt(req.params.plotIndex);
    ...
    if (plotResult.rowCount === 0) return res.status(404).json({ error: '地块不存在' });
    const plot = plotResult.rows[0];
    if (!plot.is_unlocked || !plot.seed_id || plot.stage < 4) return res.status(400).json({ error: '条件不满足' });
    let harvestItemId = plot.seed_id, harvestItemName = plot.crop_name, harvestItemIcon = plot.crop_icon, harvestItemRarity = plot.rarity;
    if (plot.crop_id) { const r = await client.query('SELECT * FROM items WHERE id = $1', [plot.crop_id]); if (r.rowCount > 0) { harvestItemId = r.rows[0].id; harvestItemName = r.rows[0].name; harvestItemIcon = r.rows[0].icon; harvestItemRarity = r.rows[0].rarity; } }
    const maxCropCount = await getConfig('max_crop_count') || 9999;
    const currentCount = await getUserItemCount(userId, harvestItemId);
    if (currentCount >= maxCropCount) return res.status(400).json({ error: '背包已满，请先清理后再收获', overflow: true });
    ...
    let actualYield = Math.max(1, Math.round(baseYield * levelMultiplier * (1 + petBonus / 100)));
    let bonusYield = 0;
    ...
    if (!accumulator[harvestItemRarity]) accumulator[harvestItemRarity] = 0;
    accumulator[harvestItemRarity] += baseYield;
    const threshold = await getConfig('yield_accumulator_threshold') || 10;
    bonusYield = Math.floor(accumulator[harvestItemRarity] / threshold);
    accumulator[harvestItemRarity] %= threshold;
    ...
    await client.query('BEGIN');
    try {
      await client.query('INSERT INTO user_items (user_id, item_id, quantity) VALUES ($1,$2,$3) ON CONFLICT (user_id, item_id) DO UPDATE SET quantity = user_items.quantity + $3, updated_at = CURRENT_TIMESTAMP', [userId, harvestItemId, actualYield]);
    ...
    const cur = await client.query('SELECT silver_coin, gold_coin, diamond FROM currencies WHERE user_id = $1', [userId]);
    const response = { message: '收获成功', plot_index: plotIndex, crop: { id: harvestItemId, name: harvestItemName, icon: harvestItemIcon, rarity: harvestItemRarity }, yield: actualYield, base_yield: baseYield, bonus_yield: bonusYield, currencies: { silver_coin: Number(cur.rows[0].silver_coin), gold_coin: Number(cur.rows[0].gold_coin), diamond: Number(cur.rows[0].diamond) } };
    if (bonusYield > 0) response.lucky_message = `🍀 幸运收获！多得了 ${bonusYield} 个`;
    if (sssDrop) response.sss_drop = sssDrop;
    res.json(response);
  } catch (error) { logger.error('Harvest error', { error: error.message }); res.status(500).json({ error: '收获失败' }); }
});

router.post('/user/plots/:plotIndex/remove', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id; const plotIndex = parseInt(req.params.plotIndex);
    const plotResult = await client.query('SELECT * FROM garden_plots WHERE user_id = $1 AND plot_index = $2', [userId, plotIndex]);
    if (plotResult.rowCount === 0) return res.status(404).json({ error: '地块不存在' });
    const plot = plotResult.rows[0];
    if (!plot.is_unlocked || !plot.seed_id) return res.status(400).json({ error: '条件不满足' });
    if (plot.stage >= 4) return res.status(400).json({ error: '成熟植物不可铲除' });
    await client.query('UPDATE garden_plots SET seed_id = NULL, stage = 0, planted_at = NULL, last_watered_at = NULL, fertilized = FALSE WHERE user_id = $1 AND plot_index = $2', [userId, plotIndex]);
    res.json({ message: '铲除成功', plot_index: plotIndex });
  } catch (error) { logger.error('Remove error', { error: error.message }); res.status(500).json({ error: '铲除失败' }); }
});
...
router.get('/garden', authenticateToken, async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: '获取花园状态失败' }); }
});
